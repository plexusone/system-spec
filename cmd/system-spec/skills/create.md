...
```bash
# Install d2: brew install d2 (macOS) or see https://d2lang.com
d2 system.d2 system.svg          # SVG output
d2 system.d2 system.png          # PNG output
d2 --watch system.d2 system.svg  # Live reload
open system.svg                   # View the diagram
```

**Mermaid → Image** (use mermaid-cli or paste into editor):
```bash
...
```bash
# The JSON output needs an HTML wrapper. Create viewer.html:
```
```html
<!DOCTYPE html>
<html>
<head>
  ...
  <style>#cy { width: 100%; height: 100vh; }</style>
</head>
<body>
  <div id="cy"></div>
  <script>
    fetch('system.cyto.json')
      .then(r => r.json())
      .then(data => cytoscape({ container: document.getElementById('cy'), elements: data }));
  </script>
</body>
</html>
```
```bash
# Then open viewer.html in a browser (needs local server for fetch):
python3 -m http.server 8000
open http://localhost:8000/viewer.html
```

**GraphViz DOT → Image** (requires graphviz):
```bash
# Install: brew install graphviz (macOS)
...
open graph.html                  # View combined code + infra graph
```

This connects services to their source code, showing which code implements which services.
