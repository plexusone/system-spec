...
An **Owner** represents a team or individual responsible for one or more services. Owners are defined at the system level and referenced by ID in services.
...
| `owners` | `map[string]Owner` | No | Team owners keyed by ID |
...
```json
{
  ...
  "services": {
    "api-gateway": { ... },
    "payments-service": { "owner": "payments-team", ... }
  }
}
```
...
| `owner` | `string` | No | Owner ID referencing `system.owners` |
...
| `envVars` | `ServiceEnvVars` | No | Environment variables for service discovery |
...
| `azure` | `AzureResources` | No | Azure resource bindings |
| `cloudflare` | `CloudflareResources` | No | Cloudflare resource bindings |
| `resources` | `GenericResources` | No | Provider-agnostic resource bindings |
...
| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `name` | `string` | Yes | Display name (e.g., "Payments Team") |
| `email` | `string` | No | Team contact email |
| `slack` | `string` | No | Slack channel or handle (e.g., "#payments-eng") |
| `pager` | `string` | No | PagerDuty/Opsgenie service or escalation policy ID |
...
### ServiceEnvVars Object

Documents environment variables for service discovery. Clients use these env vars to locate the service.

| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `url` | `string` | No | Env var name for full service URL (e.g., "PAYMENTS_URL") |
| `host` | `string` | No | Env var name for service host (e.g., "PAYMENTS_SERVICE_HOST") |
| `port` | `string` | No | Env var name for service port (e.g., "PAYMENTS_SERVICE_PORT") |
...
Note: Kubernetes automatically creates `{SVCNAME}_SERVICE_HOST` and `{SVCNAME}_SERVICE_PORT` environment variables for each service.
...
| `lambda` | `[]LambdaFunction` | Lambda functions |
| `elasticache` | `[]ElastiCacheCluster` | ElastiCache clusters (Redis/Memcached) |
| `kinesis` | `[]KinesisStream` | Kinesis data streams |
| `eventbridge` | `[]EventBridgeBus` | EventBridge event buses |
| `secretsmanager` | `[]SecretsManagerSecret` | Secrets Manager secrets |
| `apigateway` | `[]APIGatewayAPI` | API Gateway APIs |
| `cloudfront` | `[]CloudFrontDistribution` | CloudFront distributions |
...
| `cloudrun` | `[]CloudRunService` | Cloud Run services |
| `cloudfunctions` | `[]CloudFunction` | Cloud Functions |
| `memorystore` | `[]MemorystoreInstance` | Memorystore (Redis) instances |
| `bigquery` | `[]BigQueryDataset` | BigQuery datasets |
| `secretmanager` | `[]GCPSecret` | Secret Manager secrets |
| `firestore` | `[]FirestoreDatabase` | Firestore databases |
| `spanner` | `[]SpannerInstance` | Cloud Spanner instances |
| `vpcNetwork` | `string` | VPC network name |

### AzureResources Object

Contains Azure-specific resource bindings.

| Field | Type | Description |
|-------|------|-------------|
| `cosmosdb` | `[]CosmosDBAccount` | Cosmos DB accounts |
| `servicebus` | `[]ServiceBusNamespace` | Service Bus namespaces |
| `blobstorage` | `[]BlobStorageAccount` | Blob Storage accounts |
| `functions` | `[]AzureFunction` | Azure Functions apps |
| `rediscache` | `[]AzureRedisCache` | Azure Cache for Redis |
| `keyvault` | `[]AzureKeyVault` | Key Vault instances |
| `eventhubs` | `[]EventHubsNamespace` | Event Hubs namespaces |
| `sqldatabase` | `[]AzureSQLDatabase` | Azure SQL databases |
...
| `kv` | `[]CloudflareKV` | KV namespaces |
| `d1` | `[]CloudflareD1` | D1 databases |
| `queues` | `[]CloudflareQueue` | Cloudflare Queues |
| `durableObjects` | `[]CloudflareDurableObject` | Durable Object namespaces |

### GenericResources Object

Contains provider-agnostic resource bindings for self-hosted or multi-cloud deployments.

| Field | Type | Description |
|-------|------|-------------|
| `redis` | `[]RedisInstance` | Redis instances |
| `kafka` | `[]KafkaCluster` | Kafka clusters |
| `elasticsearch` | `[]ElasticsearchCluster` | Elasticsearch clusters |
| `mongodb` | `[]MongoDBInstance` | MongoDB instances |
| `postgresql` | `[]PostgreSQLDatabase` | PostgreSQL databases |
| `mysql` | `[]MySQLDatabase` | MySQL databases |
| `rabbitmq` | `[]RabbitMQInstance` | RabbitMQ instances |
| `cassandra` | `[]CassandraCluster` | Cassandra clusters |
| `memcached` | `[]MemcachedInstance` | Memcached instances |
| `minio` | `[]MinioInstance` | Minio (S3-compatible) instances |
| `nats` | `[]NATSServer` | NATS servers |
| `etcd` | `[]EtcdCluster` | etcd clusters |
| `vault` | `[]VaultInstance` | HashiCorp Vault instances |
| `consul` | `[]ConsulInstance` | HashiCorp Consul instances |
